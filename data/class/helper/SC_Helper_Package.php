<?php
class SC_Helper_Package
{
    public function sfEditPkgData($arrData)
    {
        ...
        // 新規登録
        // Package Notice Time
        $arrData['notice_time'] = 'CURRENT_TIMESTAMP';

        // Package status:未送達
        $arrData['package_status'] = 1;

        $objQuery->insert('dtb_package', $arrData);
        $objQuery->commit();

        return 0;
    }

    public function sfPkgNoticeParam(&$objFormParam)
    {

     	SC_Helper_Package::sfPkgInfoParam($objFormParam);
    	//SC_Helper_Package::sfItemInfoParam($objFormParam, $isAdmin);

    }

    public function sfPkgInfoParam(&$objFormParam, $prefix = '')
    {
    	$objFormParam->addParam('配送公司', $prefix . 'delivery_company', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK'));
    	$objFormParam->addParam('包裹单号', $prefix . 'package_no', STEXT_LEN, 'aKV', array('EXIST_CHECK'));
    	$objFormParam->addParam('备注', $prefix . 'comment', LTEXT_LEN, 'aKV', array('MAX_LENGTH_CHECK'));
    }

    public function sfPkgNoticeErrorCheck(&$objFormParam)
    {
    	$objErr = SC_Helper_Package::sfPkgInfoErrorCheck($objFormParam);
    	//$objErr = SC_Helper_Package::sfItemInfoErrorCheck($objErr);

    	return $objErr->arrErr;
    }

    public function sfPkgInfoErrorCheck(&$objFormParam, $prefix = '')
    {
...
//     }
    ...
    public function sfCustomerCommonParam(&$objFormParam, $prefix = '')
    {
    	$objFormParam->addParam('会员ID', $prefix . 'c_id', STEXT_LEN, 'n', array('NO_SPTAB', 'SPTAB_CHECK'));
    	$objFormParam->addParam('姓', $prefix . 'name01', STEXT_LEN, 'aKV', array('EXIST_CHECK', 'NO_SPTAB', 'SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
    	$objFormParam->addParam('名', $prefix . 'name02', STEXT_LEN, 'aKV', array('EXIST_CHECK', 'NO_SPTAB', 'SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
    	$objFormParam->addParam('ZIPCODE', $prefix . 'zipcode', STEXT_LEN, 'n', array('NO_SPTAB', 'SPTAB_CHECK', 'GRAPH_CHECK', 'MAX_LENGTH_CHECK'));
    	$objFormParam->addParam('省/自治区/直辖市', $prefix . 'pref', INT_LEN, 'n', array('NUM_CHECK'));
    	$objFormParam->addParam('住所1', $prefix . 'addr01', MTEXT_LEN, 'aKV', array('EXIST_CHECK', 'SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
    	$objFormParam->addParam('住所2', $prefix . 'addr02', MTEXT_LEN, 'aKV', array('EXIST_CHECK', 'SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
    	$objFormParam->addParam('电话号码', $prefix . 'tel01', TEL_ITEM_LEN, 'n', array('EXIST_CHECK', 'SPTAB_CHECK', 'NUM_CHECK'));
    }
...
//     }
    public function sfCustomerRegisterParam(&$objFormParam, $isAdmin = false, $is_mypage = false, $prefix = '')
    {
    	$objFormParam->addParam('密码', $prefix . 'password', PASSWORD_MAX_LEN, '', array('EXIST_CHECK', 'SPTAB_CHECK', 'GRAPH_CHECK'));
//     	$objFormParam->addParam('パスワード確認用の質問の答え', $prefix . 'reminder_answer', STEXT_LEN, '', array('EXIST_CHECK', 'SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
//     	$objFormParam->addParam('パスワード確認用の質問', $prefix . 'reminder', STEXT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
...
//     	$objFormParam->addParam('メールマガジン', $prefix . 'mailmaga_flg', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
    	...
    	if (SC_Display_Ex::detectDevice() !== DEVICE_TYPE_MOBILE) {
    		$objFormParam->addParam('电子邮箱', $prefix . 'email', null, 'a', array('NO_SPTAB', 'EXIST_CHECK', 'EMAIL_CHECK', 'SPTAB_CHECK', 'EMAIL_CHAR_CHECK'));
    		$objFormParam->addParam('密码(确认用)', $prefix . 'password02', PASSWORD_MAX_LEN, '', array('EXIST_CHECK', 'SPTAB_CHECK', 'GRAPH_CHECK'), '', false);
    		if (!$isAdmin) {
    			$objFormParam->addParam('电子邮箱(确认用)', $prefix . 'email02', null, 'a', array('NO_SPTAB', 'EXIST_CHECK', 'EMAIL_CHECK', 'SPTAB_CHECK', 'EMAIL_CHAR_CHECK'), '', false);
    		}
    	} else {
    		if (!$is_mypage) {
    			$objFormParam->addParam('电子邮箱', $prefix . 'email', null, 'a', array('EXIST_CHECK', 'EMAIL_CHECK', 'NO_SPTAB', 'EMAIL_CHAR_CHECK', 'MOBILE_EMAIL_CHECK'));
    		}
    	}
    }
    ...
    public function sfCustomerEntryErrorCheck(&$objFormParam)
    {
        ...
        if ($objCustomer->isLoginSuccess(true)
            ...
            $objErr->arrErr['email'] .= '※ 该电子邮箱已被登录，请换一个电子邮箱进行注册。<br />';
        }
//         if ($objCustomer->isLoginSuccess(true)
...
//         $objErr->doFunc(array('郵便番号', $prefix . 'zip01', $prefix . 'zip02'), array('ALL_EXIST_CHECK'));
        $objErr->doFunc(array('电话号码', $prefix . 'tel01'), array('EXIST_CHECK', 'NUM_CHECK'));
        $objErr->doFunc(array('邮政编码', $prefix . 'zipcode'), array('EXIST_CHECK', 'NUM_CHECK'));
    ...
    {
        $objErr->doFunc(array('生日', 'year', 'month', 'day'), array('CHECK_BIRTHDAY'));
        $objErr->doFunc(array('密码', 'password', PASSWORD_MIN_LEN, PASSWORD_MAX_LEN), array('NUM_RANGE_CHECK'));

        if (SC_Display_Ex::detectDevice() !== DEVICE_TYPE_MOBILE) {
            if (!$isAdmin) {
                $objErr->doFunc(array('电子邮箱', '电子邮箱(确认用)', 'email', 'email02'), array('EQUAL_CHECK'));
            }
            $objErr->doFunc(array('密码', '密码(确认用)', 'password', 'password02'), array('EQUAL_CHECK'));
        }

        if (!$isAdmin) {
            // 現会員の判定 → 現会員もしくは仮登録中は、メアド一意が前提になってるので同じメアドで登録不可
            $objErr->doFunc(array('电子邮箱', 'email'), array('CHECK_REGIST_CUSTOMER_EMAIL'));
